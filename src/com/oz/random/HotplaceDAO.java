...
public class HotplaceDAO {
	...
	public static void getHotplace(HttpServletRequest request) {
		...
		try {
			...
			String sql = "select * from travel_Hot where t_placeNo = ?";
	...
	public static void upd(HttpServletRequest request) {
		...
		try {
			...
			String sql = "insert into travel_Hot values(travel_Hot_seq.nextval,?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
